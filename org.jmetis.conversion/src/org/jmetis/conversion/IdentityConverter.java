...
public class IdentityConverter implements ITypeConverter {
	...
	private static Class<?>[][] primitiveMap = new Class[][] {
	...
	public <T> T convertTo(Object sourceValue, Class<T> targetType) {
		if (targetType.isPrimitive()) {
			if (sourceValue == null) {
				return targetType
						.cast(IdentityConverter.PRIMITIVE_DEFAULT_VALUES
								.get(targetType));
			}
		}
		if (sourceValue != null) {
			...
			if (targetType.isPrimitive()) {
				if (sourceClass.equals(this.targetType)
						|| this.isPrimitiveTypeMatchedWithBoxed(sourceClass,
								this.targetType)) {
			...
			if (!this.targetType.isAssignableFrom(sourceClass)) {
				throw new ConversionException(sourceClass.getName()
						+ " is not assignable to " + this.targetType.getName()); //$NON-NLS-1$
			}
		}
	...
	static {
		...
		IdentityConverter.PRIMITIVE_DEFAULT_VALUES.put(Boolean.TYPE,
				Boolean.FALSE);
		IdentityConverter.PRIMITIVE_DEFAULT_VALUES.put(Boolean.class,
				Boolean.FALSE);
		...
		IdentityConverter.PRIMITIVE_DEFAULT_VALUES
				.put(Character.TYPE, (char) 0);
		IdentityConverter.PRIMITIVE_DEFAULT_VALUES.put(Character.class,
				(char) 0);
