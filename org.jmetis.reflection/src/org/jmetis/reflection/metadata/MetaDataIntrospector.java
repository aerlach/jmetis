...
public class MetaDataIntrospector implements IMetaDataIntrospector {
	...
	private static final Map<String, Method[]> EMPTY_PROPERTY_METHODS = Collections
			.emptyMap();

	private final static Type[] EMPTY_TYPE_ARRAY = {};
	 ...
	 * 
	 * @see
	 * org.jmetis.reflection.metadata.IMetaDataIntrospector#classDescriptorOf
	 * (java.lang.Class)
	 */
	 ...
	 * 
	 * @see
	 * org.jmetis.reflection.metadata.IMetaDataIntrospector#declaredFieldsOf
	 * (java.lang.Class)
	 */
	public Field[] declaredFieldsOf(final Class<?> declaringClass)
	...
	protected Field basicDeclaredFieldNamed(Class<?> declaringClass,
	 ...
	 * 
	 * @see
	 * org.jmetis.reflection.metadata.IMetaDataIntrospector#declaredFieldNamed
	 * (java.lang.Class, java.lang.String)
	 */
	public Field declaredFieldNamed(final Class<?> declaringClass,
				...
				return AccessController
						.doPrivileged(new PrivilegedExceptionAction<Field>() {
							public Field run() throws SecurityException,
									NoSuchFieldException {
								return MetaDataIntrospector.this
										.basicDeclaredFieldNamed(
	 ...
	 * 
	 * @see
	 * org.jmetis.reflection.metadata.IMetaDataIntrospector#declaredMethodsOf
	 * (java.lang.Class)
	 */
	public Method[] declaredMethodsOf(final Class<?> declaringClass)
			throws SecurityException {
		if (System.getSecurityManager() != null) {
			return AccessController
					.doPrivileged(new PrivilegedAction<Method[]>() {
						public Method[] run() {
							return MetaDataIntrospector.this
									.basicDeclaredMethodsOf(declaringClass);
						}
					});
		}
	 ...
	 */
	protected boolean isBridgedCandidateFor(Method candidateMethod,
			Method bridgeMethod) {
	 ...
	 */
	protected boolean isBridgeMethodFor(Method bridgeMethod,
	 ...
	 */
	protected Method searchCandidates(List<Method> candidateMethods,
			Method bridgeMethod) {
	...
	public Method findBridgedMethod(Method bridgeMethod) {
		...
		Method[] methods = allDeclaredMethodsOf(bridgeMethod
				.getDeclaringClass());
	...
	protected String propertyNameFrom(Method method, int startIndex) {
		...
		return new String(characters, startIndex, characters.length
				- startIndex);
	}

	private Method[] propertyMethodsFrom(String propertyName,
			Map<String, Method[]> collectedMethods) {
	...
	public Map<String, Method[]> propertyMethodsOf(Class<?> declaringClass) {
		if (declaringClass.isAnnotation() || declaringClass.isAnonymousClass()
				|| declaringClass.isArray() || declaringClass.isEnum()
				|| declaringClass.isInterface() || declaringClass.isPrimitive()) {
			return MetaDataIntrospector.EMPTY_PROPERTY_METHODS;
		}
		Method[] declaredMethods = declaredMethodsOf(declaringClass);
		Map<String, Method[]> allPropertyMethods = new HashMap<String, Method[]>(
			...
			if (!Modifier.isStatic(modifiers) && Modifier.isPublic(modifiers)) {
				...
				if (methodName.length() >= 3) {
					...
					if (resultType == void.class) {
						if (methodName
								.startsWith(MetaDataIntrospector.SET_PREFIX)) {
							propertyMethods = propertyMethodsFrom(
									propertyNameFrom(method, 3),
									allPropertyMethods);
							propertyMethods[1] = method;
						}
					} else if (methodName
							.startsWith(MetaDataIntrospector.GET_PREFIX)) {
						propertyMethods = propertyMethodsFrom(propertyNameFrom(
								method, 3), allPropertyMethods);
						propertyMethods[0] = method;
					} else if ((resultType == boolean.class || resultType == Boolean.class)
							&& methodName
									.startsWith(MetaDataIntrospector.IS_PREFIX)) {
						propertyMethods = propertyMethodsFrom(propertyNameFrom(
								method, 2), allPropertyMethods);
						propertyMethods[0] = method;
					}
				}
			}
	...
	public Annotation[] declaredAnnotationsOf(
	...
	public <T extends Annotation> List<Method> annotatedMethodsFrom(
	 ...
	 */
	private Type getRawType(Type genericType,
	...
	protected Map<TypeVariable<?>, Type> addTypeVariableTo(
	 ...
	 */
	protected Map<TypeVariable<?>, Type> addTypeVariablesOfParameterizedTypeTo(
		...
		if (rawType instanceof Class) {
			...
			TypeVariable<?>[] typeParameters = ((Class<?>) rawType)
					.getTypeParameters();
			for (int i = 0; i < actualTypeArguments.length; i++) {
				Type actualTypeArgument = actualTypeArguments[i];
				TypeVariable<?> typeVariable = typeParameters[i];
				if (actualTypeArgument instanceof Class) {
					typeVariableMap = addTypeVariableTo(typeVariable,
							actualTypeArgument, typeVariableMap);
	...
	protected Map<TypeVariable<?>, Type> addTypeVariablesOfInterfaceTo(
			Type genericInterface, Map<TypeVariable<?>, Type> typeVariableMap) {
		if (genericInterface instanceof ParameterizedType) {
			ParameterizedType parameterizedType = (ParameterizedType) genericInterface;
			typeVariableMap = addTypeVariablesOfParameterizedTypeTo(
					parameterizedType, typeVariableMap);
			Type rawType = parameterizedType.getRawType();
			if (rawType instanceof Class) {
				typeVariableMap = addTypeVariablesOfInterfacesTo(
						(Class<?>) rawType, typeVariableMap);
			}
		} else if (genericInterface instanceof Class) {
			typeVariableMap = addTypeVariablesOfInterfacesTo(
					(Class<?>) genericInterface, typeVariableMap);
		}
		return typeVariableMap;
	}

	protected Map<TypeVariable<?>, Type> addTypeVariablesOfInterfacesTo(
			Class<?> classToDescribe, Map<TypeVariable<?>, Type> typeVariableMap) {
		for (Type genericInterface : classToDescribe.getGenericInterfaces()) {
			typeVariableMap = addTypeVariablesOfInterfaceTo(genericInterface,
					typeVariableMap);
		}
		return typeVariableMap;
	}

	protected Map<TypeVariable<?>, Type> addTypeVariablesOfSuperclassesTo(
			Class<?> classToDescribe, Map<TypeVariable<?>, Type> typeVariableMap) {
		...
		if (genericType instanceof Type) {
			...
			while (!Object.class.equals(currentClass)) {
				if (genericType instanceof ParameterizedType) {
					typeVariableMap = addTypeVariablesOfParameterizedTypeTo(
							(ParameterizedType) genericType, typeVariableMap);
				}
				typeVariableMap = addTypeVariablesOfInterfacesTo(currentClass,
						typeVariableMap);
	...
	protected Map<TypeVariable<?>, Type> addTypeVariablesOfEnclosingClassTo(
			Class<?> classToDescribe, Map<TypeVariable<?>, Type> typeVariableMap) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @seeorg.jmetis.reflection.metadata.IMetaDataIntrospector#
	 * typeVariableMapOf(java.lang.Class)
	 */
	public Map<TypeVariable<?>, Type> typeVariableMapOf(Class<?> classToDescribe) {
		...
		typeVariableMap = addTypeVariablesOfInterfacesTo(classToDescribe,
				typeVariableMap);
		typeVariableMap = addTypeVariablesOfSuperclassesTo(classToDescribe,
				typeVariableMap);
		typeVariableMap = addTypeVariablesOfEnclosingClassTo(classToDescribe,
				typeVariableMap);
		return typeVariableMap;
	}
	 ...
	 * <code>false</code>.
	 */
	protected boolean isResolvedTypeMatch(Method genericMethod,
			Method candidateMethod, Map<TypeVariable<?>, Type> typeVariableMap) {
	...
	protected Class<?> calculateComponentType(Type aType) {
		if (aType instanceof ParameterizedType) {
			Type[] actualTypeArguments = ((ParameterizedType) aType)
					.getActualTypeArguments();
			assert actualTypeArguments.length > 0 : "actualTypeArguments.length > 0"; //$NON-NLS-1$
			if (actualTypeArguments[0] instanceof Class) {
				return (Class<?>) actualTypeArguments[0];
			} else if (actualTypeArguments[0] instanceof WildcardType) {
				Type[] upperBounds = ((WildcardType) actualTypeArguments[0])
						.getUpperBounds();
				...
				assert true; // if we get here we got no upper bounds that's
				// more than we can handle.
			}
		} else if (aType instanceof TypeVariable) {
			// if we get here we got a type variable (e.g. T) don't we?
			// T is more than we can handle.
		} else if (aType instanceof GenericArrayType) {
			Type genericComponentType = ((GenericArrayType) aType)
					.getGenericComponentType();
			...
			assert true; // if we get here we got no upper bounds that's more
			// than we can handle.
		} else {
			assert false : "Unreachable code"; //$NON-NLS-1$
		}
		return null;
